...
class Process:
    ...
    def select_random_video(self, viddir, min_duration=4):
        videos = [
            f
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt == max_attempts - 1:
                    print(
                        f"ents get 1 free voucher code to use for either the Foundations or CopiloFailed to render video after {max_attempts} attempts. Skipping this folder."
                    )
                    return
                else:
