...
def generate_subtitles(
    video_path: str, beat_info: Dict, yt_title: str
) -> Optional[str]:
            ...
            subtitles.extend(
                [
                    ...
                    srt.Subtitle(
                        ...
                        content="Purchase this beat at the link in the description (Buy 2 Get 2 Free!)",
                    ),
                ]
            )
...
def upload_to_dropbox(
    file_path: str, dropbox_folder: str, dropbox_instance
) -> Optional[str]:
...
def process_folder(
    ...
    try:
        ...
        matching_beat = next(
            (beat for beat in beat_results if len(beat) >= 5 and beat[4] == folder_bpm),
            None,
        )
        ...
        uploaded_files = list(
            process_files_with_dropbox(folder_path, dropbox_folder_name, dropbox_token)
        )
...
def main():
    ...
    for folder_name in os.listdir(root_folder):
        ...
        if os.path.isdir(folder_path):
            youtube_data = process_folder(
                folder_path, channel_name, channel_config, global_config, beat_manager
            )
            if youtube_data:
    ...
    if all_youtube_data:
        save_to_csv(
            all_youtube_data, channel_name, os.path.dirname(os.path.abspath(__file__))
        )
    else:
        logging.warning(
