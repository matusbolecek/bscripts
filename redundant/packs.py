...
def add_new_beat(
    cursor, name: str, collaborators: str, key: str, tempo: int, link: str
):
    cursor.execute(
        """
...
def extract_beat_info(filename: str) -> Tuple[str, str, int, str]:
    ...
    if len(parts) < 2:
        raise ValueError(
            f"Filename '{filename}' does not contain enough parts separated by ' - '"
        )
...
def process_beat(cursor, file: Path, pack_name: str) -> Tuple:
    try:
        ...
        print(
            f"Extracted info: name='{name}', collaborators='{collaborators}', bpm={bpm}, key='{key}'"
        )
    ...
    if not name:
        print(
            f"Warning: Empty beat name extracted from '{file.name}'. Skipping this file."
        )
        return None, False
    ...
    if beat:
        ...
        print(
            f"Beat '{name}' not found in database. Please enter the following information:"
        )
...
def generate_new_filename(
    beat_number: int, name: str, bpm: int, key: str, collaborators: str
) -> str:
    # Split collaborators and remove any instance of 'matejcikbeats'
    collab_list = [
        c.strip()
        for c in collaborators.split(",")
        if c.strip().lower() != "matejcikbeats"
    ]
...
def generate_pdfs_and_merge(
    pack_name: str, beat_properties: list, output_folder: Path, resources_path: Path
):
    class PDF(FPDF):
        def footer(self):
            self.set_y(-15)
    ...
    for props in beat_properties:
        ...
        pdf.chapter_body(
            f'{track_nr}. "{pdf_props[0]}" - {pdf_props[1]} {pdf_props[2]}\n (prod. by {collab})'
        )
        if pdf_props[4]:
...
def process_beatpack(
    pack_name: str, beat_properties: list, output_folder: Path, resources_path: Path
):
    # Generate and merge PDFs
    merged_pdf = generate_pdfs_and_merge(
        pack_name, beat_properties, output_folder, resources_path
    )

    # Create THANK YOU.txt
    thank_you_file = create_thank_you_txt(
        pack_name, len(beat_properties), output_folder
    )
...
def main():
                ...
                if should_process and beat is not None:
                    ...
                    try:
                        tag_and_copy_beat(
                            ...
                            beat[3],
                            beat[2],
                        )  # beat[4] is tempo, beat[1] is name, beat[3] is key, beat[2] is collaborators
                        print(f"Processed and copied: {file.name}")

                        # Create beat_properties string
                        beat_props = (
                            f"{beat[1]};{beat[4]};{beat[3]};{beat[2]};{beat[8] or ''}"
                        )
    ...
    if selected_beats:
        process_beatpack(
            pack_name, beat_properties, output_folder, Beatpack.resources_path
        )
