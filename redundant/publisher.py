...
def load_config(
    script_dir: str, config_file: str = f"{Publisher.resources_path}/publisher.json"
) -> Dict:
...
def save_to_csv(data: List[Dict], platform: str, export_folder: str, script_dir: str):
    ...
    template_path = os.path.join(
        script_dir, f"{Publisher.resources_path}/Bulk_Uploader_Template.csv"
    )
    if platform == "youtube":
        template_path = os.path.join(
...
def process_files(folder_path: str, export_folder: str, script_dir: str):
    ...
    try:
        ...
        file_generator = process_files_with_dropbox(
            folder_path, dropbox_folder_name, Publisher.dropbox1
        )
        ...
        for filename, download_link in file_generator:
            ...
            if platform.lower() == "yt":
                youtube_data.append(
                    generate_youtube_data(config, beatname, download_link)
                )
                ...
                print(
                    f"Warning: Unknown platform '{platform}' for file {filename}. Skipping this file."
                )
...
def get_valid_folder_path() -> str:
    while True:
        ...
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            ...
            print(
                f"Error: The folder '{folder_path}' does not exist or is not a directory. Please try again."
            )
