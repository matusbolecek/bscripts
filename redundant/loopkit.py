...
def parse_loop_filename(filename: str) -> Beat:
    ...
    if "x" in name_parts:
        ...
        collaborators = ", ".join(
            [part.strip("@") for part in collaborators_parts if part != "x"]
        )
        ...
        if filename.endswith(".wav") or filename.endswith(".mp3"):
            ...
            if existing_loops:
                ...
                if existing_pack and existing_pack != "None":
                    logging.info(
                        f"Loop '{loop.name}' is already in pack '{existing_pack}'. Skipping..."
                    )
                    continue
            ...
            if video_filename:
                ...
                try:
                    ...
                    logging.info(
                        f"Generated short version for video: {output_filename}"
                    )
                except subprocess.CalledProcessError as e:
                    logging.error(
                        f"Error generating short version for {loop.name}: {str(e)}"
                    )
                    logging.error(f"FFmpeg stderr: {e.stderr}")
            else:
                logging.warning(
                    f"No matching file found in video folder for: {filename}"
                )

    if not youtube_loops:
        logging.warning(
            "No matching files were found in the video folder. SRT and timestamps will be empty."
        )
    else:
...
def find_matching_file(filename, folder):
    ...
    for file in os.listdir(folder):
        ...
        if match:
            ...
            if all(
                any(base_part in file_part for file_part in file_parts)
...
def generate_youtube_timestamps(loops, output_folder):
    ...
    with open(timestamp_path, "w") as f:
        ...
        for loop in loops:
            f.write(
                f"{format_time(current_time, youtube=True)} - {loop.name} ({loop.key})\n"
            )
...
if __name__ == "__main__":
    full_folder = (
        input("Enter the path to the full loops folder: ").strip("'").strip('"')
    )
    video_folder = (
        input("Enter the path to the video loops folder: ").strip("'").strip('"')
    )
