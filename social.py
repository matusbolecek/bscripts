...
for folder in listdir_nohidden(video_folder):
    ...
    for file in folder_path.iterdir():
        print(file)
        if fnmatch.fnmatch(file.name, '*.mov'):
            video_path = file
    ...
    for picture_path in pics:
        ...
        [1:v][v]overlay=-325:480,hue=h={random.randint(-360, 360)}" \
        -c:v libx264 -crf 17 -b:a 320k -preset slow "{beat_properties[0]}_temp.mp4"'
        )

        subprocess.run(ffmpeg, shell = True, executable="/bin/bash")

        ffmpeg_fin = str(f'ffmpeg -ss 0.1 -i "{beat_properties[0]}_temp.mp4" -c copy "{folder_path}/{beat_properties[0]}_{Path(picture_path).stem}.mp4"')
