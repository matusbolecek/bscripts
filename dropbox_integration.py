...
def get_dropbox_service(access_token):
    ...
    try:
        dbx = dropbox.Dropbox(access_token, timeout=300)  # Increased timeout to 5 minutes
        ...
        with open(file_path, 'rb') as f:
            ...
            while f.tell() < file_size:
                if (file_size - f.tell()) <= CHUNK_SIZE:
                    ...
                    chunks_uploaded += 1
                    logging.info(f"Uploaded final chunk {chunks_uploaded} ({min(CHUNK_SIZE, file_size - f.tell() + CHUNK_SIZE)} bytes)")
                else:
                    # Intermediate chunk
                    ...
                    chunks_uploaded += 1
                    logging.info(f"Uploaded chunk {chunks_uploaded} ({CHUNK_SIZE} bytes)")
                    
                    # Add a progress indicator every 10 chunks
                    if chunks_uploaded % 10 == 0:
                        progress = (f.tell() / file_size) * 100
                        logging.info(f"Upload progress: {progress:.1f}% ({f.tell()} / {file_size} bytes)")
...
def upload_file(dbx, file_path, folder_path):
    ...
    if upload_successful:
        try:
            ...
            direct_link = shared_link.url.replace('www.dropbox.com', 'dl.dropboxusercontent.com').replace('?dl=0', '?dl=1')
