...
keep_attributes = input('Do you want to keep the original file name as part of the output? (Y/n)')
...
for file in files:
    ...
    if keep_attributes == 'y' or keep_attributes == 'Y' or keep_attributes == '':
        finalname = str(f'{keyword} - {newname} - {Path(file).stem} @matejcikbeats{Path(file).suffix}')
    else:
